...
Agent-Steered SDLC now uses slug-only IDs for specs and plans.
...
Spec and plan IDs use `KIND-AREA-NAME`. `AREA` and `NAME` are uppercase slug tokens,
2-32 characters each, using `A-Z` and digits only after the first character. Do not use
trailing numbers or internal hyphens.
...
- Design IDs: unchanged `LAYER-SLUG`, `COMP-SLUG`, `IFACE-SLUG`, `DEC-SLUG`,
  `RISK-SLUG`.
...
4. Do not rename design IDs unless they already violate design's `KIND-SLUG` convention.
...
- Coverage below 100%: an acceptance test, work item, PR, or executable test still points
  at the old ID or omits the new one.
