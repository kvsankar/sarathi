...
appropriate after substantial artifact edits, after refactors, after resolving review
findings, and before a handoff where accumulated small issues would make the next agent or
reviewer work harder.

The pass is scoped. It improves the current artifact or planned touch set; it does not
...
record a finding, follow-up, or upstream artifact revision need instead of slipping it into
the current slice.
...
- Traceability: IDs or maps that claim coverage without a real oracle, stale `covers` claims,
...
- Documentation/build/deploy/config: stale commands, undocumented behavior changes, dead
  examples, invalid links, obsolete flags, generated artifacts that should not be checked in,
...
- Traceability theater: exact IDs are present, but the linked artifact, test, or review note
  would not fail or change if the behavior were wrong.

Remove theater when it is inside scope. Replace it with a concrete oracle, real boundary
...
1. Re-scan touched code, tests, docs, config, and traceability for the issues above.
2. Fix in-scope oddities, redundancy, misleading claims, and theater.
3. Avoid cosmetic churn outside the planned touch set.
4. If cleanup changes behavior, contracts, UX, NFRs, or planned scope, stop for upstream
