...
GitHub Release and attaches the package. Keep the npm trusted publisher bound to
`kvsankar/sarathi`, `release.yml`, and the `npm` environment. Do not configure an npm token;
the workflow authenticates through OIDC.
