...
| Cross-scope test ownership | Product, feature, and slice `AT-`/`JT-` intent at the appropriate granularity. | Executable `TEST-` obligations and oracles for local, feature-composition, and product-composition evidence. | Ancestor obligations allocated to child work/PRs, including explicit integration leaves when evidence spans children. | Code-ready leaves implement assigned ancestor/local tests; execution results remain distinct from traceability claims. |
