...
1. Load `docs/project-entry.md`.
2. Inspect enough repo files to classify adoption mode and existing artifacts.
3. Record or update `.sdlc/process-decisions.yaml` when the user chooses or approves the
   mode.
4. Load only the selected stage prompt.
5. Load deeper docs or checker sources only when the stage reaches that concern.
